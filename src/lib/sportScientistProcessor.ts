...
export async function querySportScientistDocuments(query: string): Promise<SearchResult[]> {
  console.log("=== Starting Sport Scientist Query Process ===");
  ...
  try {
    ...
    const index = pc.index(import.meta.env.VITE_PINECONE_SPORT_SCIENTIST_INDEX!);
    ...
    const results = await index.namespace('sleep-research').query({
    ...
    console.error("=== Sport Scientist Query Process Error ===");
